...
const canonical=join(source,'contextforge');const text=await readFile(join(canonical,'SKILL.md'),'utf8');
assert.match(text,/^---\r?\nname: contextforge\r?\ndescription: .+/);assert.match(text,/license: MIT/);assert.match(text,/version: "0.5.0"/);
...
assert.ok(text.includes('Never run installation silently'));assert.ok(text.includes('CLI has no status command'));assert.ok(!text.includes('contextforge status'));
assert.ok(!/[A-Z]:[\\/](?:Users|aiwork)/.test(text));
for(const m of text.matchAll(/\]\((references\/[^)]+)\)/g))await access(join(canonical,m[1]));
const temp=await realpath(await mkdtemp(join(tmpdir(),'contextforge-skill-')));
...
try{
 const discovered=run(['add',source,'--list']);assert.ok(discovered.includes('contextforge'));
 run(['add',source,'--skill','contextforge','--agent','codex','cursor','claude-code','--copy','--yes']);
 const list=run(['list','--json']);assert.ok(list.includes('contextforge'));
 for(const target of ['.agents','.claude']){
  const installed=join(temp,target,'skills/contextforge');assert.equal(await readFile(join(installed,'SKILL.md'),'utf8'),text);
 ...
 const prompt=run(['use',source,'--skill','contextforge']);assert.ok(prompt.includes('contextforge'));assert.ok(prompt.includes('SKILL.md'));
 const nativeDiscovery = process.env.CONTEXTFORGE_CODEX_BINARY ? await verifyCodexDiscovery(process.env.CONTEXTFORGE_CODEX_BINARY, temp, text) : 'NOT TESTED';
