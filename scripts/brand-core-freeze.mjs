...
assert.deepEqual(paths.filter((path) => !allowed.has(path)), [], "RepoBound migration may change only audited public-display adapters within production source; benchmark files are frozen");
const auditedPatch = execFileSync("git", ["diff", "--binary", "--no-ext-diff", base, "--", ...[...allowed].sort()], { windowsHide: true });
...
console.log(JSON.stringify({ gate: "RepoBound v0.5.1 production and benchmark freeze", base, changedProductionPaths: paths, auditedPatchSha256, compiler: "UNCHANGED", ranking: "UNCHANGED", pack: "UNCHANGED", gold: "UNCHANGED", v2Promotion: "NONE", databaseSchema: "UNCHANGED", mcpTools: "UNCHANGED", passed: true }));
